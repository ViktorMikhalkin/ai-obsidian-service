...
def _extractive(hits: List[SearchHit]) -> Tuple[str, List[Citation]]:
    cits = [Citation(doc_path=h.doc_path, chunk_id=h.chunk_id, snippet=h.preview, span=None) for h in hits[:6]]
    if not cits:
        return ("No relevant excerpts found. Rebuild the index or refine your query.", [])
    ans = "Found excerpts (extractive):\n" + "\n".join(f"- {c.snippet}  [{c.doc_path}]" for c in cits)
    return ans, cits

def answer_with_citations(query: str, hits: List[SearchHit]) -> Tuple[str, List[Citation]]:
    if not hits: return ("No relevant excerpts found.", [])
    if _MODE == "extractive": return _extractive(hits)
