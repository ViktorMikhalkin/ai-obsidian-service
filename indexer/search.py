...
_meta = [json.loads(l) for l in _meta_path.read_text().splitlines()] if _meta_path.exists() else []
...
def search(query: str, top_k: int = 8):
    ...
    for rank, rid in enumerate(idx[0]):
        if rid < 0 or rid >= len(_meta): continue
        m = _meta[rid]
        s = float(scores[0][rank])
        hits.append(SearchHit(doc_path=m["path"], chunk_id=str(m["id"]), score=s, preview=m["preview"]))
    return hits
